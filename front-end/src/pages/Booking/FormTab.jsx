...
const HealthCheckForm = ({ onTabChange }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Gọi API để tạo appointment mới
      await appointmentService.createAppointment(eventId, formData);
      
      // Cập nhật profile để chỉ ra rằng user đã có appointment
      await authService.updateProfile({ hasAppointment: true });
      ...
      message.error("Đã xảy ra lỗi khi đăng ký hiến máu: " + (error.message || "Vui lòng thử lại sau"));
    } finally {
      setLoading(false);
    }
  };
