...
const HealthCheckEdit = () => {
  ...
  // Simulate fetching existing health check data
  useEffect(() => {
    const mockData = {
      heartRate: 75,
      bloodPressure: 110,
      ...
      height: 1.7,
      weight: 65,
      notes: "Sample notes for this health check.",
      type: 1, // "Đạt"
    };
    ...
    // Calculate initial BMI and Health Metrics
    const calculatedBmi = (mockData.weight / (mockData.height * mockData.height)).toFixed(2);
    setBmi(calculatedBmi);
    setHealthMetrics(
      `${mockData.heartRate} / ${mockData.bloodPressure} / ${mockData.temperature} / ${mockData.height} / ${mockData.weight}`
    );
  }, [form]);
  ...
  const onSubmit = (values) => {
    ...
    message.success("Health check updated successfully!");
    setTimeout(() => {
  ...
  return (
    <>
      ...
      <Form
        form={form} // Gắn form instance để quản lý giá trị
        className="mt-6"
        layout="vertical"
        onFinish={onSubmit}
